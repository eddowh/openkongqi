...
class TestRecordsWrapper(unittest.TestCase):
    ...
    def load_records(self, filemod):
        """Load records in database, return the records"""
        mod = import_module('tests.data.' + filemod)
        recs = mod.DATA
        self.wrapper.write_records(recs)
        return recs

    def test_get_records(self):
        recs = self.load_records('pm25in_sh_201607130254_201607130454')
        uuid = 'pm25in:cn:shanghai:putuo'
        start = datetime(2016, 7, 13, 3)  # should query 03:54 and 04:54
        end = datetime(2016, 7, 14)
        res = self.wrapper.get_records(uuid, start, end)
        res = list(res)
        # two different records for two timestamps
        self.assertEqual(len(res), 2)
        self.assertEqual(
            res[-1]['fields']['pm25'],
            recs[uuid][-1]['fields']['pm25']
        )
